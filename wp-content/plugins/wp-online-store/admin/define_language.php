...
*/
  ...
  if(isset($_POST['lngdir'])) $HTTP_GET_VARS['lngdir']=$_POST['lngdir'];
  ...
  if (isset($HTTP_GET_VARS['filename'])) {
	...
	$replace_chard=array('\\','\\\\');
    $file_edit= str_replace($replace_chard,"/",$file_edit);

   $new_DIR_FS_CATALOG_LANGUAGES=str_replace("//","/",DIR_FS_CATALOG_LANGUAGES);
    if (substr($file_edit, 0, strlen($new_DIR_FS_CATALOG_LANGUAGES)) != $new_DIR_FS_CATALOG_LANGUAGES) {
      ...
      case 'save':
        if (isset($HTTP_GET_VARS['lngdir']) && isset($HTTP_GET_VARS['filename'])) {
          ...
          if (file_exists($file) && tep_is_writable($file)) {
            ...
            $file_contents = stripslashes($_REQUEST['file_contents']);
           // echo '<textarea>'.$file_contents.'</textarea>';die('d');
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr><?php echo tep_draw_form('lng', FILENAME_DEFINE_LANGUAGE, '', 'post'); ?>
