...
*/
  ...
  if(isset($_POST['search'])) $HTTP_GET_VARS['search']=$_POST['search'];

  if(isset($_POST['filter'])) $HTTP_GET_VARS['filter']=$_POST['filter'];

  if(isset($_POST['module']))	$HTTP_GET_VARS['module']=$_POST['module'];
...
<?php
  echo tep_draw_form('search', FILENAME_ACTION_RECORDER, '', 'post');
...
?>
              ...
              <tr>
                <td class="smallText" align="right">
<?php
  echo tep_draw_form('filter', FILENAME_ACTION_RECORDER, '', 'post');
...
<?php
  ...
  $actions_split = new splitPageResults($HTTP_GET_VARS['pages'], MAX_DISPLAY_SEARCH_RESULTS, $actions_query_raw, $actions_query_numrows);
...
?>
              <tr>
                <td colspan="5"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $actions_split->display_count($actions_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['pages'], TEXT_DISPLAY_NUMBER_OF_ENTRIES); ?></td>
                    <td class="smallText" align="right"><?php echo $actions_split->display_links($actions_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $HTTP_GET_VARS['pages'], (isset($HTTP_GET_VARS['module']) && in_array($HTTP_GET_VARS['module'], $modules_array) && is_object(${$HTTP_GET_VARS['module']}) ? 'module=' . $HTTP_GET_VARS['module'] : null) . '&' . (isset($HTTP_GET_VARS['search']) && !empty($HTTP_GET_VARS['search']) ? 'search=' . $HTTP_GET_VARS['search'] : null),'pages'); ?></td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
