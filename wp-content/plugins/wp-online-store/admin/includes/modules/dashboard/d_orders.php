...
*/
  ...
  class d_orders {
    ...
    function getOutput() {
      ...
      $output = '
	             <div class="round_box_nav">' .
                '  <ul>' .
                '    <li>' . MODULE_ADMIN_DASHBOARD_ORDERS_TITLE . '</li>' .
                '    <li>' . MODULE_ADMIN_DASHBOARD_ORDERS_TOTAL . '</li>' .
                '    <li>' . MODULE_ADMIN_DASHBOARD_ORDERS_DATE . '</li>' .
                '    <li style="border-right:0px;">' . MODULE_ADMIN_DASHBOARD_ORDERS_ORDER_STATUS . '</li>' .
                '  </ul></div>';

      $output .= '<div class="round_box_cnt"> 
                  <table border="0" cellspacing="3"  cellpadding="3" width="100%" style="border:1px solid #dfdfdf;" rules="all">';				
      ...
      while ($orders = tep_db_fetch_array($orders_query)) {
        $output .= '  <tr>' .
                   '    <td><a href="' . tep_href_link(FILENAME_ORDERS, 'oID=' . (int)$orders['orders_id'] . '&action=edit') . '">' . tep_output_string_protected($orders['customers_name']) . '</td>' .
                   '    <td>' . strip_tags($orders['order_total']) . '</td>' .
                   '    <td>' . tep_date_short($orders['date_last_modified']) . '</td>' .
                   '    <td style="border-right:0px;">' . $orders['orders_status_name'] . '</td>' .
                   '  </tr>';
      }
