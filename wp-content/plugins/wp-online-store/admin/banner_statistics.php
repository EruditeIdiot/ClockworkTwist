...
*/
 ...
 // we cant use get in form posting
  if(isset($HTTP_POST_VARS['type']))
  	$HTTP_GET_VARS['type'] = $HTTP_POST_VARS['type'] ;
  	
  if(isset($HTTP_POST_VARS['month']))
  	$HTTP_GET_VARS['month'] = $HTTP_POST_VARS['month'] ;

  if(isset($HTTP_POST_VARS['year']))
  	$HTTP_GET_VARS['year'] = $HTTP_POST_VARS['year'] ;
  	
  if(isset($HTTP_POST_VARS['bID']))
  	$HTTP_GET_VARS['bID'] = $HTTP_POST_VARS['bID'] ;	
...
// check if the graphs directory exists
  ...
  if (function_exists('imagecreate') && tep_not_null($banner_extension)) {
    ...
    if (is_dir(DIR_FS_ADMIN.DIR_WS_IMAGES . 'graphs')) {

      if (tep_is_writable(DIR_FS_ADMIN.DIR_WS_IMAGES . 'graphs')) {
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      ...
      <tr>
        ...
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          ...
          <tr><?php echo tep_draw_form('year', FILENAME_BANNER_STATISTICS, '', 'post'); ?>
...
<?php
    ...
    include(DIR_WS_INCLUDES.DIR_WS_FUNCTIONS . 'html_graphs.php');
