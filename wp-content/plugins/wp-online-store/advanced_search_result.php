  ...
  foreach ($epf as $e) {
    if ($e['search'])  // only advanced searchable fields will have separate values
      if (isset($HTTP_GET_VARS[$e['field']]) && !empty($HTTP_GET_VARS[$e['field']])) {
        if ($e['uses_list'] && !$e['multi_select'] && ($HTTP_GET_VARS[$e['field']] == 'on')) {
          $HTTP_GET_VARS[$e['field']] = '';
        } else {
      		$epf_empty = false;
      		if (!$e['uses_list']) 
        		$epf_values[] = tep_db_prepare_input($HTTP_GET_VARS[$e['field']]);
        }
      }
  }
...
foreach ($epf as $e) {
  if ($e['search']) { // only process advanced searchable fields
    $value = '';
    if (isset($HTTP_GET_VARS[$e['field']]) && !empty($HTTP_GET_VARS[$e['field']]))
      $value = tep_db_prepare_input($HTTP_GET_VARS[$e['field']]); // get value passed from advanced_search.php
    if ($e['uses_list'] && ($value != '')) {
      if ($e['multi_select']) {
        if (is_array($value)) {
          $match = $HTTP_GET_VARS['match' . $e['id']];
          $where_str .= " and (";
          $first = true;
      ...
      switch ($HTTP_GET_VARS['sort']) {
        case 'products_new':
         $order_str = " order by p.products_date_added  desc" ;
          break;
        case 'products_old':
          $order_str = " order by p.products_date_added  asc" ;
          break;
        case 'products_viewed':
         $order_str =  " order by p.products_ordered desc, pd.products_name";
          break;
