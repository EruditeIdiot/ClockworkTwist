...
*/
  ...
  chdir('../../../../');
  require(WP_PLUGIN_DIR . '/'.WPOLS_PLUGINS_DIR.'/includes/application_top.php');
...
// initialize variables if the customer is not logged in
  ...
  require(WP_PLUGIN_DIR . '/'.WPOLS_PLUGINS_DIR.'/includes/modules/payment/paypal_express.php');
...
// select cheapest shipping method
		...
		$ppe_token=$_SESSION['ppe_token'] = $response_array['TOKEN'];        

        if (!tep_session_is_registered('ppe_payerid')) tep_session_register('ppe_payerid');
        $ppe_payerid =$_SESSION['ppe_payerid']= $response_array['PAYERID'];

        if (!tep_session_is_registered('ppe_payerstatus')) tep_session_register('ppe_payerstatus');
        $ppe_payerstatus =$_SESSION['ppe_payerstatus']= $response_array['PAYERSTATUS'];

        if (!tep_session_is_registered('ppe_addressstatus')) tep_session_register('ppe_addressstatus');
        $ppe_addressstatus=$_SESSION['ppe_addressstatus'] = $response_array['ADDRESSSTATUS'];
...
// don't recalculate currency values as they have already been calculated
      ...
      $orig_amt=$params['AMT'] = $paypal_express->format_raw($params['ITEMAMT'] + $params['TAXAMT'] + $params['SHIPPINGAMT'], '', 1);
      ...
      if (MODULE_ORDER_TOTAL_DISCOUNT_STATUS == 'true') { 
			      // Discount Code - start
				include(WP_PLUGIN_DIR . '/'.WPOLS_PLUGINS_DIR.'/includes/modules/order_total/ot_discount.php');
				$ot_discount = new ot_discount;
				$ot_discount->process();
				$params['AMT'] = $paypal_express->format_raw($order->info['total'], '', 1);
				// Discount Code - end
			
				if($orig_amt>$params['AMT']){
					$discount=$orig_amt-$params['AMT'];
					
					$params['L_NAME' . $line_item_no] = 'Discount';
			        $params['L_AMT' . $line_item_no] = -1*($paypal_express->format_raw($discount));
			        $params['L_QTY' . $line_item_no] = 1;
			        $params['ITEMAMT']=$params['ITEMAMT']+(-1*($paypal_express->format_raw($discount)));
				}
      }
      
      
      
  	if (MODULE_ORDER_TOTAL_QUANTITY_DISCOUNT_STATUS == 'true') { 
			      // Discount Code - start
				include(WP_PLUGIN_DIR . '/'.WPOLS_PLUGINS_DIR.'/includes/modules/order_total/ot_quantity_discount.php');
				$ot_quantity_discount = new ot_quantity_discount;
				$ot_quantity_discount->process();
				$params['AMT'] = $paypal_express->format_raw($order->info['total'], '', 1);
				// Discount Code - end
