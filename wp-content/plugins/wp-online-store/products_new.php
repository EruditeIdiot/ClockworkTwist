...
?>
      <tr>
        ...
        <td valign="top" class="main"><?php echo '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products_new['products_id']) . '"><strong><u>' . $products_new['products_name'] . '</u></strong></a><br />' . TEXT_DATE_ADDED . ' ' . tep_date_long($products_new['products_date_added']) . '<br />' . TEXT_MANUFACTURER . ' ' . $products_new['manufacturers_name'] . '<br /><br />' .  ( (strpos($products_price,'<del></del>')===false) && trim($products_price)!=""?TEXT_PRICE:"") . ' ' . $products_price; ?></td>
        <td align="right" valign="middle" class="smallText"><?php if (SHOW_BUTTON == 'True') { echo tep_draw_button(IMAGE_BUTTON_IN_CART, 'cart', tep_href_link(FILENAME_PRODUCTS_NEW, tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $products_new['products_id']));} ?></td>
      </tr>
<?php
    }
?>
...
?>
    ...
    <div>
      <span style="float: right;"><?php echo TEXT_RESULT_PAGE . ' ' . $products_new_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y')),FILENAME_PRODUCTS_NEW); ?></span>
