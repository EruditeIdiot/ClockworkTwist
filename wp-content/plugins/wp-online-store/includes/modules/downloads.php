...
if (!strstr($_SERVER["REQUEST_URI"], FILENAME_ACCOUNT_HISTORY_INFO)) {
...
// - The expiry date is not reached
      ...
      if ( ($downloads['download_count'] > 0) && (file_exists(DIR_FS_DOWNLOAD . $downloads['orders_products_filename'])) && ( ($downloads['download_maxdays'] == 0) || ($download_timestamp > time())) ) {
        ...
        echo '        <td><a href="' . tep_href_link(FILENAME_DOWNLOAD, 'wpols_order=' . $last_order . '&wpols_id=' . $downloads['orders_products_download_id']) . '">' . $downloads['products_name'] . '</a></td>' . "\n";
...
<?php
    ...
    if (!strstr($_GET['slug'], FILENAME_ACCOUNT_HISTORY_INFO)) {
